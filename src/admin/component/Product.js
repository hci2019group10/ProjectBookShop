...
const Product = ()=>{
   ...
   const [totalRecords, setTotalRecords] = useState(51);
   ...
   useEffect(()=>{
    ...
    //  }
     ...
     const deleteProduct = async(productID, productDetailID) =>{
        ...
        const response2 = await customFetch(URLDeleteProductDetail, "DELETE", null);
    ...
    <tr>
        ...
        <td className="uneditable admin_img"><img src={product.mainImage}/></td>
        <td className="uneditable center">{product.price}đ</td>
        <td className="uneditable center">{getAuthor(product.author)?.authorName}</td>
        <td className="center">Kim Đồng</td>
        <td className="uneditable">{getCategory(product.category)?.categoryName}</td>
        <td><Link to={`/products/productDetail/${product.id}`}><i className="fa fa-eye"></i></Link></td>
        <td className="uneditable"><span>
            <i className="fa fa-times-circle" onClick={()=>deleteProduct(product.id, product.productDetail.id)}></i>
            </span></td>
    </tr>
    );
                    ...
                    <div>
                        <div className="row w3-res-tb">
                            <div className="col-sm-3 m-b-xs">
                                <select class="input-sm form-control w-sm inline v-middle" onChange={sortProductByCategory}>
                            ...
                            <div className="col-sm-4 m-b-xs">
                                <Link to="addProduct"><button className="btn btn-sm btn-default">Thêm<i class="fa fa-plus"></i></button></Link>
                            </div>
                        </div>
                        <table className="col-xs-12 col-lg-12" id="editable" className="pure-table pure-table-bordered">
                            <tr>
                                ...
                                <td title="Nhà xuất bản" className="uneditable">Nhà XB</td>
